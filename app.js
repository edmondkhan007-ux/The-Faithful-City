...
const STORAGE_KEY = 'faithful-city-doxa-portal-family-v9-cloud-cache';
const APP_VERSION = 9;
...
function getMemberMetrics(member) {
  ...
  const eligible = getSundayRecords()
    .filter(record => isMemberEligibleOnDate(member, record.date))
  ...
  const attendedRecords = state.services
    .filter(record => record.presentMemberIds.includes(member.id))
    .sort((a, b) => b.date.localeCompare(a.date));
  const lastAttended = attendedRecords[0]?.date || null;

  let status = 'new';
  if (opportunities >= settings.learningMinimumServices) {
    status = rate >= settings.activeThreshold && consecutiveAbsences < settings.inactiveConsecutive ? 'active' : 'inactive';
  }

  let carePriority = 'none';
  if (consecutiveAbsences >= settings.inactiveConsecutive || (opportunities >= 3 && rate < 35)) {
    carePriority = 'high';
  } else if (status === 'inactive' || consecutiveAbsences >= 2) {
    carePriority = 'medium';
  } else if (consecutiveAbsences === 1 && opportunities >= 3) {
    carePriority = 'watch';
  }

  return { rate, presentCount, opportunities, consecutiveAbsences, lastAttended, status, carePriority };
}
...
function renderDashboard() {
  ...
  document.getElementById('dashboardStats').innerHTML = [
    statCard('Current Members', getCurrentMembers().length, `${activeCount} active • ${inactiveCount} inactive`, '♙'),
  ...
  document.getElementById('dashboardCareList').innerHTML = careMembers.length ? careMembers.slice(0, 4).map(({ member, metrics }) => {
    ...
    return `<button class="care-person care-person-button" data-member-care="${member.id}">
      ...
      <div class="care-info"><strong>${escapeHTML(member.name)}</strong><span>${metrics.consecutiveAbsences} consecutive absence${metrics.consecutiveAbsences === 1 ? '' : 's'} • ${metrics.rate}% attendance${shepherd ? ` • ${escapeHTML(shepherd.name)}` : ''}</span></div>
...
function renderMembers() {
  ...
  document.getElementById('memberStats').innerHTML = [
    ...
    statCard('Active', active, `Automatic rule: ≥ ${state.settings.activeThreshold}%`, '✓'),
    statCard('Inactive', inactive, 'Suggested for review and care', '♡'),
  ...
  document.getElementById('membersTableBody').innerHTML = filtered.length ? filtered.map(member => {
    ...
    return `<tr data-status="${memberMetrics.status}">
      ...
      <td>${lifecycle === 'current' ? `<span class="priority ${statusClass(memberMetrics.status)}">${statusLabel(memberMetrics.status)}</span>` : `<span class="priority ${ministryStatusClass(lifecycle)}">${ministryStatusLabel(lifecycle)}</span>`}</td>
      <td><button class="row-action" data-member-view="${member.id}">View</button></td>
    </tr>`;
...
function renderPeriodFilters() {
  ...
  if (summary) summary.textContent = current ? `Activity classification uses ${semesterFullLabel(current)} Sunday services.` : 'No current semester selected.';
}
...
function saveAttendanceRecord() {
  ...
  showToast(`Saved ${service}: ${stats.present} members + ${stats.visitors} visitors = ${stats.total} total${topicText}.`);
}
...
function renderPrograms() {
  ...
  statsContainer.innerHTML = [
    ...
    statCard('Special Program Days', specialAttendanceRecords.length, 'Each day keeps its own attendance and ministry notes', '✓')
  ].join('');
  ...
  specialContainer.innerHTML = specialPrograms.length ? specialPrograms.map(program => {
    ...
    return `<article class="special-program-card ${program.active === false ? 'archived-program' : ''}">
      ...
      <div class="button-row program-actions">
        ...
        <button class="btn btn-soft" data-program-edit="${program.id}">Edit</button>
  ...
  openModal({
    ...
    body: `<form class="lux-form" id="memberForm">
      <div class="form-grid two-col">
        ...
        <label><span>Date of birth *</span><input required type="date" name="dob" value="${escapeHTML(existing?.dob || '')}"></label>
        <label><span>Date joined *</span><input required type="date" name="joinedDate" value="${escapeHTML(existing?.joinedDate || isoToday())}"></label>
...
function renderSettings() {
  ...
  if (summary) summary.textContent = `Current members are classified Active within ${semesterFullLabel(getCurrentSemester())} when they attend at least ${state.settings.activeThreshold}% of the latest ${state.settings.windowSize} Sunday services and have fewer than ${state.settings.inactiveConsecutive} consecutive absences. Former or archived members are excluded automatically.`;
...
function openRuleSettings() {
  openModal({
    ...
    title: 'Adjust activity rule',
    subtitle: 'These settings determine how members are automatically classified from Sunday-service attendance.',
    body: `<form class="lux-form" id="ruleSettingsForm">
      <div class="form-grid two-col">
        <label><span>Active attendance threshold (%)</span><input type="number" min="1" max="100" name="activeThreshold" value="${state.settings.activeThreshold}"></label>
        <label><span>Attendance window (latest Sundays)</span><input type="number" min="3" max="52" name="windowSize" value="${state.settings.windowSize}"></label>
        <label><span>Consecutive absences triggering inactivity</span><input type="number" min="2" max="12" name="inactiveConsecutive" value="${state.settings.inactiveConsecutive}"></label>
        <label><span>Learning period (minimum opportunities)</span><input type="number" min="1" max="8" name="learningMinimumServices" value="${state.settings.learningMinimumServices}"></label>
      </div>
      <div class="form-help">A member becomes inactive when the recent attendance rate falls below the threshold or consecutive absences reach the configured limit. New members are protected from premature classification during the learning period.</div>
    ...
    onOpen: () => {
      document.getElementById('ruleSettingsForm').addEventListener('submit', event => {
        event.preventDefault();
        const data = new FormData(event.currentTarget);
        state.settings.activeThreshold = Number(data.get('activeThreshold'));
        state.settings.windowSize = Number(data.get('windowSize'));
        state.settings.inactiveConsecutive = Number(data.get('inactiveConsecutive'));
        state.settings.learningMinimumServices = Number(data.get('learningMinimumServices'));
        saveState();
        closeModal();
        renderAll();
        showToast('Automatic activity rule updated.');
      });
    }
  });
}
