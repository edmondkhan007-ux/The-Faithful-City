...
const motionState = {
  calm: localStorage.getItem('churchcare-motion-mode') === 'calm',
  observer: null,
...
function finishCinematicLoader() {
  ...
  if (!loader) return;
  window.setTimeout(() => loader.classList.add('is-finished'), prefersReducedMotion() ? 0 : 1450);
  window.setTimeout(() => loader.remove(), prefersReducedMotion() ? 10 : 2250);
}

function splitMotionHeading(element) {
...
function getMotionObserver() {
  if (motionState.observer) return motionState.observer;
  if (!('IntersectionObserver' in window)) {
    motionState.observer = { observe(element) { element.classList.add('is-revealed'); }, unobserve() {} };
    return motionState.observer;
  }
  motionState.observer = new IntersectionObserver(entries => {
...
function animateCountElement(element) {
  ...
  const duration = Math.min(1050, 420 + target * 10);
  ...
  const tick = now => {
    ...
    if (progress < 1) requestAnimationFrame(tick);
      ...
      window.setTimeout(() => element.classList.remove('is-counting'), 600);
    }
  };
...
function bindTiltCard(card) {
  if (!card || card.dataset.tiltBound === 'true' || prefersReducedMotion() || matchMedia('(pointer:coarse)').matches) return;
  ...
  card.addEventListener('pointermove', event => {
    ...
    card.style.setProperty('--tilt-y', `${(px - .5) * 5.5}deg`);
    card.style.setProperty('--tilt-x', `${(.5 - py) * 5.5}deg`);
...
function bindMagneticElement(element) {
  if (!element || element.dataset.magneticBound === 'true' || prefersReducedMotion() || matchMedia('(pointer:coarse)').matches) return;
...
function refreshCinematicMotion(root = document) {
  if (prefersReducedMotion()) return;
  const activePage = document.querySelector('.page-section.active') || root;
  activePage.querySelectorAll('.hero h3, .page-heading h3').forEach(splitMotionHeading);
  activePage.querySelectorAll('.motion-heading').forEach(heading => {
    heading.classList.remove('is-revealed');
    requestAnimationFrame(() => heading.classList.add('is-revealed'));
  });

  const revealSelector = [
    '.page-heading > *', '.hero > *', '.current-period-summary', '.stat-card', '.panel',
    '.program-card', '.special-program-card', '.shepherd-card', '.classification-rule-banner > div',
    '.programs-section-heading', '.report-brand-header', '.motion-marquee'
  ].join(',');
  activePage.querySelectorAll(revealSelector).forEach((item, index) => {
    item.classList.add('motion-reveal');
    item.style.setProperty('--reveal-delay', `${Math.min(index, 10) * 45}ms`);
    item.classList.remove('is-revealed');
    getMotionObserver().observe(item);
  });

  activePage.querySelectorAll('.panel,.stat-card,.program-card,.special-program-card,.shepherd-card,.classification-rule-banner>div,.program-reminder-item').forEach(bindTiltCard);
  activePage.querySelectorAll('.btn,.icon-btn,.row-action').forEach(bindMagneticElement);
  activePage.querySelectorAll('.bar-report').forEach(report => {
    report.querySelectorAll('i').forEach((bar, index) => bar.style.setProperty('--bar-index', index));
  });
  activePage.querySelectorAll('.stat-card > strong,.attendance-headcount > strong,.live-counts strong').forEach(animateCountElement);

  activePage.querySelectorAll('tbody tr,.attendance-row,.care-person,.birthday-item,.high-attendance-item,.insight-list > div,.program-reminder-item').forEach((row, index) => {
    row.classList.add('motion-row');
    row.style.setProperty('--row-delay', `${Math.min(index, 14) * 35}ms`);
...
function scheduleMotionRefresh() {
  clearTimeout(window.__churchCareMotionRefresh);
  window.__churchCareMotionRefresh = setTimeout(() => refreshCinematicMotion(), 45);
}

function setMotionMode(calm) {
  motionState.calm = calm;
  document.body.classList.toggle('motion-calm', calm);
  document.getElementById('motionModeBtn')?.classList.toggle('is-calm', calm);
  localStorage.setItem('churchcare-motion-mode', calm ? 'calm' : 'cinematic');
  showToast(calm ? 'Calm motion enabled.' : 'Cinematic motion enabled.');
}
...
function setupPointerScene() {
  ...
  document.addEventListener('pointermove', event => {
    ...
    const hero = document.querySelector('.page-section.active .hero');
    const seal = hero?.querySelector('.hero-logo-seal');
    if (hero && seal) {
      const rect = hero.getBoundingClientRect();
      const px = (event.clientX - rect.left) / rect.width - .5;
      const py = (event.clientY - rect.top) / rect.height - .5;
      seal.style.transform = `rotateY(${px * 10}deg) rotateX(${-py * 9}deg) translate3d(${px * 8}px,${py * 8}px,18px)`;
    }
  }, { passive: true });
...
function setupCinematicMotion() {
  ...
  document.body.classList.toggle('motion-calm', motionState.calm);
  document.getElementById('motionModeBtn')?.classList.toggle('is-calm', motionState.calm);
  document.getElementById('motionModeBtn')?.addEventListener('click', () => setMotionMode(!motionState.calm));
  setupPointerScene();
  window.addEventListener('load', finishCinematicLoader, { once: true });
  window.setTimeout(finishCinematicLoader, 2200);
...
function switchPage(pageName) {
  ...
  if (!target || motionState.pageTransitioning) return;
  const current = document.querySelector('.page-section.active');
  const performSwitch = () => {
    document.querySelectorAll('.page-section').forEach(page => {
      page.classList.remove('active', 'cinematic-enter');
    });
    target.classList.add('active', 'cinematic-enter');
    ...
    renderAll();
    scheduleMotionRefresh();
    window.setTimeout(() => target.classList.remove('cinematic-enter'), 900);
  };
  ...
  motionState.pageTransitioning = true;
  const wipe = document.getElementById('cinematicPageWipe');
  const label = document.getElementById('pageWipeLabel');
  if (label) label.textContent = String(pageName).replaceAll('-', ' ').toUpperCase();
  wipe?.classList.add('is-active');
  window.setTimeout(performSwitch, 340);
  window.setTimeout(() => {
    wipe?.classList.remove('is-active');
    motionState.pageTransitioning = false;
  }, 900);
}
