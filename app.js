...
function classificationMemberCard(member, classification) {
  const shepherd = getMemberShepherd(member);
  const missText = classification.sundayMisses || classification.thursdayMisses
    ? `${classification.sundayMisses} Sun • ${classification.thursdayMisses} Thu`
    : 'No current consecutive misses';
  return `<article class="classification-member-card ${classificationClass(classification.zone)}">
    <button class="classification-member-profile" data-member-view="${member.id}" type="button">
      <span class="member-avatar">${initials(member.name)}</span>
      <span><strong>${escapeHTML(member.name)}</strong><small>${escapeHTML(shepherd?.name || (member.role === 'shepherd' ? 'Registered shepherd' : 'No shepherd assigned'))}</small></span>
    </button>
    <div class="classification-member-foot"><span>${escapeHTML(missText)}</span><button class="mini-edit-btn" data-edit-member="${member.id}" type="button">Edit</button></div>
  </article>`;
}
...
function renderClassification() {
  ...
  const alertHtml = alerts.length
    ? `<div class="admin-alert-icon">!</div><div><strong>Admin attention: ${alerts.length} member${alerts.length === 1 ? '' : 's'} need follow-up.</strong><p>${counts('red')} urgent red • ${counts('green')} needs-help green. Review the remarks below and contact the responsible shepherd.</p></div>`
  ...
  const filtered = all.filter(({member,classification}) => {
    const shepherd = getMemberShepherd(member);
    const haystack = `${member.name} ${member.phone || ''} ${shepherd?.name || ''} ${classification.reason} ${classification.adminMessage}`.toLowerCase();
    return haystack.includes(query);
  });

  const renderZone = (zone, targetId, countId, emptyText) => {
    const items = filtered.filter(item => item.classification.zone === zone);
    const target = document.getElementById(targetId);
    const countTarget = document.getElementById(countId);
    const totalForZone = counts(zone);
    if (countTarget) countTarget.textContent = query ? `${items.length} of ${totalForZone} members` : `${totalForZone} member${totalForZone === 1 ? '' : 's'}`;
    if (target) target.innerHTML = items.length
      ? items.map(({member,classification}) => classificationMemberCard(member, classification)).join('')
      : `<div class="classification-zone-empty">${escapeHTML(emptyText)}</div>`;
  };

  renderZone('active', 'classificationActiveColumn', 'classificationActiveCount', 'No regular members match this search.');
  renderZone('green', 'classificationGreenColumn', 'classificationGreenCount', 'No Green members match this search.');
  renderZone('red', 'classificationRedColumn', 'classificationRedCount', 'No Red members match this search.');

  const unclassified = filtered.filter(item => item.classification.zone === 'unclassified');
  const unclassifiedNotice = document.getElementById('classificationUnclassifiedNotice');
  if (unclassifiedNotice) {
    unclassifiedNotice.innerHTML = unclassified.length
      ? `<div><strong>Awaiting classification: ${unclassified.length}</strong><p>${unclassified.map(({member}) => escapeHTML(member.name)).join(' • ')}</p></div><span>These members need more attendance history or an administrator review.</span>`
      : '';
    unclassifiedNotice.hidden = unclassified.length === 0;
  }

  const remarks = filtered
    .filter(item => item.classification.zone !== 'active')
    .sort((a,b) => {
      const rank = { red: 3, green: 2, unclassified: 1 };
      return (rank[b.classification.zone] || 0) - (rank[a.classification.zone] || 0) || a.member.name.localeCompare(b.member.name);
    });
  const remarksBody = document.getElementById('classificationRemarksBody');
  if (remarksBody) remarksBody.innerHTML = remarks.length ? remarks.map(({member,classification}) => {
    const shepherd = getMemberShepherd(member);
    const remark = classification.adminMessage || classification.reason || 'Needs administrator review.';
    return `<tr class="classification-row ${classificationClass(classification.zone)}">
      ...
      <td><span class="classification-pill ${classificationClass(classification.zone)}">${classificationLabel(classification.zone)}</span></td>
      <td><span class="admin-message">${escapeHTML(remark)}</span></td>
  ...
  }).join('') : '<tr><td colspan="7"><div class="empty-state">No Green, Red or unclassified members match this search.</div></td></tr>';
}
...
function saveAttendanceRecord() {
  ...
  showToast(`Saved ${service}: ${stats.present} members + ${stats.visitors} visitors = ${stats.total} total${topicText}. The all-meetings Excel report now includes this record.${alertText}`);
}
...
function getAllMeetingAttendanceData() {
  const records = [...state.services].sort((a, b) => a.date.localeCompare(b.date) || String(a.service || '').localeCompare(String(b.service || '')));
  const summaryRows = records.map((record, index) => {
    ...
    return {
      ...
      'Day': record.date ? parseISODate(record.date).toLocaleDateString('en-GB', { weekday: 'long' }) : '',
      'Academic Year': semester?.academicYear || record.academicYear || '',
      'Semester': semester?.name || '',
      'Calendar Year': record.calendarYear || record.date?.slice(0, 4) || '',
      'Service / Program': record.service || '',
      'Meeting Type': programTypeLabel(program?.type || record.eventType),
      'Topic Ministered': record.topic || '',
      'Minister / Speaker': record.minister || '',
      'Scripture / Reference': record.scripture || '',
      'Members Present': stats.present,
      'Members Absent': stats.absent,
      'Visitors / New': stats.visitors,
      'Total Attendance': stats.total,
      'Member Attendance Rate': `${stats.rate}%`,
      'Notes': record.notes || '',
  ...
  records.forEach(record => {
    const semester = getRecordSemester(record);
    const program = getProgramForRecord(record);
    const eligibleMembers = state.members.filter(member => isMemberEligibleOnDate(member, record.date)).sort((a, b) => a.name.localeCompare(b.name));
    eligibleMembers.forEach(member => {
      ...
      memberRows.push({
        ...
        'Phone': member.phone || '',
        'Shepherd': shepherd?.name || (member.role === 'shepherd' ? 'Registered shepherd' : 'Unassigned'),
        'Attendance': (record.presentMemberIds || []).includes(member.id) ? 'Present' : 'Absent'
      });
    });
  });
  ...
  records.forEach(record => {
    ...
    (record.visitorIds || []).forEach(visitorId => {
      ...
      visitorRows.push({
        ...
        'Phone': visitor.phone || '',
        'Invited By': visitor.invitedByName || '',
        'First Visit': visitor.firstVisit || ''
      });
    });
  });

  return { records, summaryRows, memberRows, visitorRows };
}

function fitWorksheetColumns(sheet, rows, maximumWidth = 34) {
  if (!rows.length) return;
  const keys = Object.keys(rows[0]);
  sheet['!cols'] = keys.map(key => {
    const width = Math.min(maximumWidth, Math.max(12, key.length + 2, ...rows.slice(0, 250).map(row => String(row[key] ?? '').length + 2)));
    return { wch: width };
  });
...
function exportAllMeetingsExcel() {
  const { records, summaryRows, memberRows, visitorRows } = getAllMeetingAttendanceData();
  if (!records.length) return showToast('No attendance records have been saved yet.');
  if (!window.XLSX) {
    showToast('The Excel library did not load. A CSV report will be downloaded instead.');
    return exportAttendanceCsv();
  }

  const workbook = XLSX.utils.book_new();
  workbook.Props = {
    Title: 'FAITHFUL CITY DOXA PORTAL FAMILY — All Meetings Attendance',
    Subject: 'Updated attendance workbook generated from ChurchCare',
    Author: 'FAITHFUL CITY DOXA PORTAL FAMILY',
    CreatedDate: new Date()
  };

  const totalAttendance = summaryRows.reduce((sum, row) => sum + Number(row['Total Attendance'] || 0), 0);
  const overviewRows = [
    ['FAITHFUL CITY DOXA PORTAL FAMILY'],
    ['ChurchCare All-Meetings Attendance Workbook'],
    [],
    ['Generated', new Date().toLocaleString('en-GB')],
    ['Total recorded meetings', records.length],
    ['Total attendance across meetings', totalAttendance],
    ['Latest meeting date', records[records.length - 1]?.date || ''],
    ['Workbook contents', 'Meeting Summary, Member Attendance, Visitor Attendance'],
    [],
    ['Important', 'Download this workbook again after each meeting to get the latest Supabase attendance data.']
  ];
  const overviewSheet = XLSX.utils.aoa_to_sheet(overviewRows);
  overviewSheet['!cols'] = [{ wch: 28 }, { wch: 72 }];
  XLSX.utils.book_append_sheet(workbook, overviewSheet, 'Overview');

  const summarySheet = XLSX.utils.json_to_sheet(summaryRows);
  fitWorksheetColumns(summarySheet, summaryRows, 38);
  XLSX.utils.book_append_sheet(workbook, summarySheet, 'Meeting Summary');

  const memberTemplate = [{ 'Meeting Date': '', 'Academic Year': '', 'Semester': '', 'Service / Program': '', 'Meeting Type': '', 'Member Name': '', 'Phone': '', 'Shepherd': '', 'Attendance': '' }];
  const memberSheetRows = memberRows.length ? memberRows : memberTemplate;
  const memberSheet = XLSX.utils.json_to_sheet(memberSheetRows);
  fitWorksheetColumns(memberSheet, memberSheetRows, 32);
  XLSX.utils.book_append_sheet(workbook, memberSheet, 'Member Attendance');

  const visitorSheet = XLSX.utils.json_to_sheet(visitorRows.length ? visitorRows : [{ 'Meeting Date': '', 'Academic Year': '', 'Semester': '', 'Service / Program': '', 'Meeting Type': '', 'Visitor Name': '', 'Phone': '', 'Invited By': '', 'First Visit': '' }]);
  fitWorksheetColumns(visitorSheet, visitorRows.length ? visitorRows : [{ 'Meeting Date': '', 'Academic Year': '', 'Semester': '', 'Service / Program': '', 'Meeting Type': '', 'Visitor Name': '', 'Phone': '', 'Invited By': '', 'First Visit': '' }], 32);
  XLSX.utils.book_append_sheet(workbook, visitorSheet, 'Visitor Attendance');

  XLSX.writeFile(workbook, `faithful-city-all-meetings-attendance-${isoToday()}.xlsx`, { compression: true });
  showToast(`Excel downloaded with ${records.length} meeting record${records.length === 1 ? '' : 's'}.`);
}

function renderMasterAttendanceSummary() {
  const target = document.getElementById('masterAttendanceSummary');
  if (!target) return;
  const { records, summaryRows } = getAllMeetingAttendanceData();
  const totalAttendance = summaryRows.reduce((sum, row) => sum + Number(row['Total Attendance'] || 0), 0);
  const semesters = new Set(summaryRows.map(row => `${row['Academic Year']} ${row['Semester']}`.trim()).filter(Boolean));
  const latest = records.length ? records[records.length - 1] : null;
  const latestStats = latest ? getRecordStats(latest) : null;
  target.innerHTML = `<div><span>Recorded meetings</span><strong>${records.length}</strong></div><div><span>Semesters represented</span><strong>${semesters.size}</strong></div><div><span>Total headcount recorded</span><strong>${totalAttendance}</strong></div><div><span>Latest meeting</span><strong>${latest ? `${formatDate(latest.date)} • ${latestStats.total} people` : 'No record'}</strong></div>`;
}
...
document.getElementById('exportAllMeetingsExcel')?.addEventListener('click', exportAllMeetingsExcel);
document.getElementById('exportAllMeetingsExcelSecondary')?.addEventListener('click', exportAllMeetingsExcel);
document.getElementById('exportAllMeetingsExcelHistory')?.addEventListener('click', exportAllMeetingsExcel);
